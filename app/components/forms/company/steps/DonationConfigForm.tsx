...
import { ArrowUpCircle, Percent, Gift, CircleDollarSign } from "lucide-react"
    ...
    >
      ...
      <motion.div variants={itemAnimation} className="space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {DONATION_METHODS.map(({ id, title, description, icon }) => (
            <motion.button
              key={id}
              type="button"
              ...
              <div className="relative z-10">
                <div className="mb-4 flex justify-center">
                  {React.createElement(icon, {
                    className: `h-12 w-12 ${
                      donationMethods.includes(id) ? "text-blue-600" : "text-gray-400"
                    }`
                  })}
                </div>
